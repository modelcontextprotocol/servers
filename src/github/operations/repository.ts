import { z } from "zod";
import { githubRequest, baseUrl } from "../common/utils.js";
...
export async function createRepository(options: CreateRepositoryOptions) {
  const response = await githubRequest(`${baseUrl}/user/repos`, {
    method: "POST",
    body: options,
  });
...
export async function forkRepository(
  ...
  const url = organization
    ? `${baseUrl}/repos/${owner}/${repo}/forks?organization=${organization}`
