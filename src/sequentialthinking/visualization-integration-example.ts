...
export function addVisualizationCapabilities(server: Server, sessionsDir: string) {
  
  // Define the visualization tool
  const VISUALIZATION_TOOL: Tool = {
...
async function handleVisualizationRequest(args: any, sessionsDir: string) {
  // Validate arguments
  if (!args.sessionId || !args.format) {
    throw new McpError(
      ErrorCode.InvalidParams,
      "Missing required parameters: sessionId and format"
    );
  }
  ...
  if (!["html", "mermaid", "json"].includes(args.format)) {
    throw new McpError(
      ErrorCode.InvalidParams,
      "Invalid format. Must be one of: html, mermaid, json"
    );
  }
  ...
  try {
    const sessionData = JSON.parse(fs.readFileSync(sessionFilePath, 'utf8')) as SessionData;
