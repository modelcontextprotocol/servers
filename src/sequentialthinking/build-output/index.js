...
import { LIST_TEMPLATES_TOOL, GET_TAGS_TOOL, GET_TEMPLATE_TOOL, CREATE_FROM_TEMPLATE_TOOL, SAVE_TEMPLATE_TOOL, DELETE_TEMPLATE_TOOL, handleListTemplatesRequest, handleGetTagsRequest, handleGetTemplateRequest, handleCreateFromTemplateRequest, handleSaveTemplateRequest, handleDeleteTemplateRequest } from './template-tools.js';
import { VALIDATE_THINKING_TOOL, GENERATE_THOUGHT_TOOL, GET_COACHING_TOOL, GET_AI_ADVICE_TOOL, handleValidateThinkingRequest, handleGenerateThoughtRequest, handleGetCoachingRequest, handleGetAIAdviceRequest } from './ai-tools.js';
...
class SequentialThinkingServer {
     ...
     */
    initializeFromTemplate(templateId, parameters = {}) {
        try {
    ...
    validateThoughtData(input) {
        ...
        if (data.validationStatus !== undefined &&
            !['valid', 'invalid', 'uncertain'].includes(data.validationStatus)) {
        ...
        return {
            ...
            isChainOfThought: data.isChainOfThought,
            isHypothesis: data.isHypothesis,
            isVerification: data.isVerification,
            chainOfThoughtStep: data.chainOfThoughtStep,
            totalChainOfThoughtSteps: data.totalChainOfThoughtSteps,
            // New fields for enhancements
            confidenceLevel: data.confidenceLevel,
            hypothesisId: data.hypothesisId,
            mergeBranchId: data.mergeBranchId,
            mergeBranchPoint: data.mergeBranchPoint,
    ...
    validateChainOfThought(thought) {
    ...
    // Merge branches
    mergeBranches(sourceBranchId, targetBranchId, mergePoint) {
    ...
    formatThought(thoughtData) {
        const { thoughtNumber, totalThoughts, thought, isRevision, revisesThought, branchFromThought, branchId, isChainOfThought, isHypothesis, isVerification, chainOfThoughtStep, totalChainOfThoughtSteps, 
        // New fields
        confidenceLevel, hypothesisId, mergeBranchId, mergeBranchPoint, validationStatus, validationReason } = thoughtData;
        let prefix = '';
        let context = '';
        let additionalInfo = '';
        if (isChainOfThought) {
            if (isHypothesis) {
                ...
                if (validationStatus) {
                    const statusColor = validationStatus === 'valid' ? chalk.green :
    ...
    processThought(input) {
        try {
            ...
            if (validatedInput.mergeBranchId && validatedInput.branchId && validatedInput.mergeBranchPoint) {
                this.mergeBranches(validatedInput.mergeBranchId, validatedInput.branchId, validatedInput.mergeBranchPoint);
            }
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    if (request.params.name === "sequentialthinking") {
        if (!request.params.arguments) {
            throw new McpError(ErrorCode.InvalidParams, "Missing arguments for sequentialthinking");
        }
        return thinkingServer.processThought(request.params.arguments);
    }
    else if (request.params.name === "visualize_thinking") {
        if (!request.params.arguments) {
            throw new McpError(ErrorCode.InvalidParams, "Missing arguments for visualize_thinking");
        }
        return handleVisualizationRequest(request.params.arguments, SAVE_DIR, thinkingServer.sessionId, thinkingServer.thoughtHistory, thinkingServer.branches);
    }
        ...
        if (!request.params.arguments) {
            throw new McpError(ErrorCode.InvalidParams, "Missing arguments for get_template");
        }
        ...
        if (!request.params.arguments) {
            throw new McpError(ErrorCode.InvalidParams, "Missing arguments for create_from_template");
        }
        const args = request.params.arguments;
        const result = handleCreateFromTemplateRequest(args, thinkingServer);
        // Initialize the session from the template
        if (args.templateId) {
            thinkingServer.initializeFromTemplate(args.templateId, args.parameters || {});
        }
        return result;
    }
    else if (request.params.name === "save_template") {
        if (!request.params.arguments) {
            throw new McpError(ErrorCode.InvalidParams, "Missing arguments for save_template");
        }
        return handleSaveTemplateRequest(request.params.arguments);
    }
    else if (request.params.name === "delete_template") {
        if (!request.params.arguments) {
            throw new McpError(ErrorCode.InvalidParams, "Missing arguments for delete_template");
        }
