...
export const SAVE_TEMPLATE_TOOL = {
    name: "save_template",
...
export const DELETE_TEMPLATE_TOOL = {
 ...
 */
export function handleListTemplatesRequest(args) {
    try {
        let templates;
        if (args.category) {
        ...
        throw new McpError(ErrorCode.InternalError, `Error listing templates: ${error instanceof Error ? error.message : String(error)}`);
    }
}
/**
...
export function handleGetTagsRequest() {
    try {
        ...
        throw new McpError(ErrorCode.InternalError, `Error getting tags: ${error instanceof Error ? error.message : String(error)}`);
    }
}
/**
 * Handle get template request
 */
export function handleGetTemplateRequest(args) {
    try {
        if (!args.templateId) {
            throw new McpError(ErrorCode.InvalidParams, "Missing required parameter: templateId");
        }
        ...
        throw new McpError(ErrorCode.InternalError, `Error getting template: ${error instanceof Error ? error.message : String(error)}`);
    }
}
/**
 * Handle create from template request
 */
export function handleCreateFromTemplateRequest(args, thinkingServer) {
    try {
        if (!args.templateId) {
            throw new McpError(ErrorCode.InvalidParams, "Missing required parameter: templateId");
        }
        ...
        throw new McpError(ErrorCode.InternalError, `Error creating from template: ${error instanceof Error ? error.message : String(error)}`);
    }
}
/**
 * Handle save template request
 */
export function handleSaveTemplateRequest(args) {
    try {
        if (!args.template) {
        ...
        try {
            ...
            throw new McpError(ErrorCode.InvalidParams, `Invalid template format: ${error instanceof Error ? error.message : String(error)}`);
        }
        return {
            content: [{
                    type: "text",
        ...
        throw new McpError(ErrorCode.InternalError, `Error saving template: ${error instanceof Error ? error.message : String(error)}`);
    }
}
/**
 * Handle delete template request
 */
export function handleDeleteTemplateRequest(args) {
    try {
        if (!args.templateId) {
            throw new McpError(ErrorCode.InvalidParams, "Missing required parameter: templateId");
        }
        ...
        if (!success) {
            throw new McpError(ErrorCode.InvalidParams, `Template not found or cannot be deleted: ${args.templateId}`);
        }
        return {
            content: [{
                    type: "text",
        ...
        throw new McpError(ErrorCode.InternalError, `Error deleting template: ${error instanceof Error ? error.message : String(error)}`);
    }
}
