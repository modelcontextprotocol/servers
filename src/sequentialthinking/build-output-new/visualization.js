 ...
 */
export async function handleVisualizationRequest(args, sessionsDir, currentSessionId, thoughtHistory, branches) {
    // Validate arguments
    if (!args.format) {
    ...
    if (!["mermaid", "json"].includes(args.format)) {
        throw new McpError(ErrorCode.InvalidParams, "Invalid format. Must be one of: mermaid, json");
    }
    try {
        let sessionData;
