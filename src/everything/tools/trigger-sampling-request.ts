...
import { McpServer, CallToolResult, CreateMessageRequest } from '@modelcontextprotocol/server';
import { z } from 'zod';
...
export const registerTriggerSamplingRequestTool = (server: McpServer) => {
    ...
    const clientSupportsSampling: boolean = clientCapabilities.sampling !== undefined;
    ...
    if (clientSupportsSampling) {
        server.registerTool(name, config, async (args, ctx): Promise<CallToolResult> => {
            ...
            const result = await ctx.mcpReq.send(request, CreateMessageResultSchema);
