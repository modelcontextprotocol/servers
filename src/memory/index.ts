...
// Simple in-memory lock manager to prevent race conditions
// Uses a Promise-based queue to serialize file operations
class LockManager {
  private locks: Map<string, Promise<void>> = new Map();

  async withLock<T>(key: string, operation: () => Promise<T>): Promise<T> {
    // Get the current lock promise for this key, or start with a resolved promise
    const currentLock = this.locks.get(key) ?? Promise.resolve();
    
    // Create a new lock that waits for the current lock to complete before starting
    const newLock = currentLock.then(() => operation());
    
    // Store the lock promise (without the operation result) for future operations
    // This ensures the next operation waits for this one to complete
    this.locks.set(key, newLock.then(() => undefined));
...
export class KnowledgeGraphManager {
  ...
  private async executeWithLock<T>(operation: () => Promise<T>): Promise<T> {
    return lockManager.withLock(this.memoryFilePath, operation);
  }
