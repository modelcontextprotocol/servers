...
export async function ensureBrowser(): Promise<BrowserContext> {
  if (context && !context.pages()[0]?.isClosed()) return context;
...
export async function openPage(): Promise<PageHandle> {
...
export async function gotoGhl(page: Page, path: string): Promise<void> {
  ...
  for (let i = 0; i < attempts; i++) {
    try {
      ...
      if (isLoginUrl(finalUrl)) {
        throw new Error(
          "GHL session expired or not logged in. Run `npm run login` to refresh the stored browser state.",
        );
      }
      return;
    } catch (err) {
      ...
      const errMsg = (err && typeof err === "object" && "message" in err && typeof err.message === "string")
        ? err.message
        : String(err);
      if (errMsg.includes("session expired")) throw err;
...
export async function screenshotError(page: Page, label: string): Promise<string> {
  if (!existsSync(SCREENSHOTS_DIR)) mkdirSync(SCREENSHOTS_DIR, { recursive: true });
