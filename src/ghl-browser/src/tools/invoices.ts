...
export const invoiceModule: ToolModule = {
  tools: [
    {
      name: "ghl_browser_list_invoices",
      description:
        "List invoices with contact, amount, status (draft/sent/paid/overdue/void), " +
    ...
    },
    {
      ...
      description:
        "Create a new invoice. Specify contact, line items (description, quantity, rate), " +
      ...
      inputSchema: {
        ...
        properties: {
          ...
          lineItems: {
            type: "string",
            description:
              "Semicolon-separated line items: 'description|qty|rate;description|qty|rate'. " +
    ...
    },
    {
      ...
      description:
        "Open an invoice and return its full details: line items, amounts, tax, " +
    ...
    },
    {
      ...
      description:
        "Send an invoice to the contact via email. Optionally include a custom message.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description:
        "Record a payment against an invoice. Specify amount, payment method, date, and reference.",
      inputSchema: {
        type: "object",
        properties: {
          invoiceNumber: { type: "string" },
    ...
    },
    {
      ...
      description:
        "Void or delete an invoice. Voided invoices remain in the system but are marked inactive.",
      inputSchema: {
        type: "object",
        properties: {
          invoiceNumber: { type: "string" },
  ...
  handlers: {
      ...
      try {
        return await withPageError(page, "invoices-details", async () => {
          ...
          const details = await page.evaluate(() => {
            const lineItems: Array<{ description: string; qty: string; rate: string; total: string }> = [];
            ...
            const payments: Array<{ amount: string; method: string; date: string; reference: string }> = [];
    ...
    ghl_browser_void_invoice: async (args) => {
      ...
      if (!confirm) throw new Error("confirm must be true to void/delete an invoice");
      if (!number && !id) throw new Error("invoiceNumber or invoiceId is required");
