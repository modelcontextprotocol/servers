...
export const tagsFieldsModule: ToolModule = {
  tools: [
    {
      name: "ghl_browser_list_tags",
      description:
        "List all tags configured in the sub-account. Returns tag name, color, and usage count.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description:
        "Create a new tag in the sub-account.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description:
        "Delete a tag. This removes it from all contacts.",
      inputSchema: {
        type: "object",
        properties: {
          tagName: { type: "string" },
    ...
    },
    {
      ...
      description:
        "List all custom fields for the sub-account. Returns field name, key, type, and model (contact/opportunity).",
      inputSchema: {
        type: "object",
        properties: {
          model: {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          fieldType: {
            type: "string",
            description:
              "Field type: 'text', 'textarea', 'number', 'phone', 'email', 'date', 'checkbox', 'radio', 'select', 'multiselect', 'url'",
          },
          model: {
            type: "string",
    ...
    },
    {
      ...
      description:
        "Update a custom field's name, placeholder, options, or settings.",
      inputSchema: {
        type: "object",
        properties: {
  ...
  handlers: {
      ...
      try {
        return await withPageError(page, "tags-list", async () => {
            ...
            for (const sel of rowSelectors) {
              document.querySelectorAll(sel).forEach((el) => {
                const nameEl = el.querySelector(
                  "td:first-child, [class*='name'], [class*='Name'], h3, span",
                );
                const colorEl = el.querySelector(
                  "[class*='color'], [class*='Color'], [class*='swatch'], [style*='background']",
                );
                const countEl = el.querySelector(
                  "[class*='count'], [class*='Count'], td:nth-child(3)",
                );
                const bgColor =
                  colorEl?.getAttribute("style")?.match(/background(?:-color)?:\s*([^;]+)/)?.[1]?.trim() || "";
          ...
          const deduped = Array.from(
            new Map(tags.map((t) => [t.id || t.name, t])).values(),
          );
    ...
    ghl_browser_delete_tag: async (args) => {
      ...
      if (!confirm) throw new Error("confirm must be true to delete a tag");
      if (!name && !id) throw new Error("tagName or tagId is required");
        ...
        return await withPageError(page, "custom-fields-list", async () => {
          ...
          const fields = await page.evaluate(() => {
            ...
            for (const sel of rowSelectors) {
              document.querySelectorAll(sel).forEach((el) => {
                const nameEl = el.querySelector(
                  "td:first-child, [class*='name'], [class*='Name'], h3",
                );
                const keyEl = el.querySelector(
                  "[class*='key'], [class*='Key'], code, td:nth-child(2)",
                );
                const typeEl = el.querySelector(
                  "[class*='type'], [class*='Type'], td:nth-child(3)",
                );
                const modelEl = el.querySelector(
                  "[class*='model'], [class*='Model'], td:nth-child(4)",
                );
                const requiredEl = el.querySelector(
                  "[class*='required'], [aria-required], td:has([class*='check'])",
                );
                const id = el.getAttribute("data-row-key") || el.getAttribute("data-id") || "";
                items.push({
                  name: nameEl?.textContent?.trim() || "",
          ...
          const deduped = Array.from(
            new Map(fields.map((f) => [f.id || f.key || f.name, f])).values(),
          );
      ...
      try {
        return await withPageError(page, "custom-fields-create", async () => {
          ...
          const typeSelect = page
            .locator(
              'select[name*="type"], [class*="type"] [class*="select"], [class*="Type"] [class*="select"]',
            )
            .first();
          try {
          ...
          if (required) {
            const requiredToggle = page
              .locator(
                'label:has-text("Required") input, [class*="required"] [class*="toggle"], input[name="required"]',
              )
              .first();
            try {
    ...
    ghl_browser_delete_custom_field: async (args) => {
      ...
      const id = asString(args.fieldId);
      const confirm = Boolean(args.confirm);
      if (!confirm) throw new Error("confirm must be true to delete a custom field");
      if (!name && !id) throw new Error("fieldName or fieldId is required");
