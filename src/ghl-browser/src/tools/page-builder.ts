...
export const pageBuilderModule: ToolModule = {
  tools: [
    {
      name: "ghl_browser_list_sites",
      description:
        "List websites and funnels on the GHL Sites index page. " +
    ...
    },
    {
      ...
      description:
        "Get the page tree for a site or funnel: list all pages with name, URL path, " +
    ...
    },
    {
      ...
      description:
        "Open the WYSIWYG page builder for a specific page and return the current element tree: " +
    ...
    },
    {
      ...
      description:
        "Add a section or element to a page in the builder. " +
    ...
    },
    {
      ...
      description:
        "Edit properties of an element on a page: text content, link URL, image source, " +
    ...
    },
    {
      ...
      description:
        "Save changes in the page builder. Optionally publish to make the page live.",
      inputSchema: {
        type: "object",
        properties: {
          siteName: { type: "string" },
      ...
      try {
        return await withPageError(page, "sites-list", async () => {
          ...
          const sites = await page.evaluate(() => {
            const items: Array<{ name: string; type: string; pageCount: string; status: string; href: string }> = [];
            const rowSelectors = [
              "tr[data-row-key]",
              '[class*="SiteRow"]',
              '[class*="site-row"]',
      ...
      try {
        return await withPageError(page, "sites-page-tree", async () => {
          ...
          const tree = await page.evaluate(() => {
            const pages: Array<{ name: string; path: string; status: string; isHome: boolean; href: string }> = [];
            const rowSelectors = [
              "tr[data-row-key]",
              '[class*="PageRow"]',
              '[class*="page-row"]',
      ...
      try {
        return await withPageError(page, "page-builder-add-section", async () => {
          ...
          const elementOption = page
            .locator(
              `[class*="element"]:has-text("${elementType}"), ` +
