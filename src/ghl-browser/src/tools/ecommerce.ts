...
export const ecommerceModule: ToolModule = {
  tools: [
    {
  ...
  handlers: {
      ...
      try {
        return await withPageError(page, "ecommerce-products", async () => {
          ...
          const rows = await page.evaluate(() => {
            const items: Array<{ name: string; price: string; inventory: string; status: string }> = [];
    ...
    ghl_browser_create_product: async (args) => {
      ...
      const type = (args.type as string) || "digital";
      const { page, close } = await openPage();
      try {
    ...
    ghl_browser_list_orders: async (args) => {
      ...
      try {
        return await withPageError(page, "ecommerce-orders", async () => {
          ...
          const rows = await page.evaluate(() => {
            const items: Array<{ id: string; contact: string; amount: string; status: string; date: string }> = [];
