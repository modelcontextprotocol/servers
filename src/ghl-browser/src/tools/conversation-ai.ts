...
export const conversationAiModule: ToolModule = {
  tools: [
    {
      name: "ghl_browser_get_conversation_ai_config",
      description:
        "Get the current Conversation AI / chatbot configuration for the sub-account: " +
    ...
    },
    {
      ...
      description:
        "Update Conversation AI settings: bot name, tone/personality, enabled channels, " +
    ...
    },
    {
      ...
      description:
        "List knowledge base / training data entries used by Conversation AI: " +
    ...
    },
    {
      ...
      description:
        "Add a knowledge base entry: FAQ, document text, URL to crawl, or custom Q&A pair.",
      inputSchema: {
        type: "object",
        properties: {
          type: {
    ...
    },
    {
      ...
      description:
        "Get recent Conversation AI chat logs: contact name, channel, messages, and resolution status.",
      inputSchema: {
        type: "object",
        properties: {
  ...
  handlers: {
        ...
        return await withPageError(page, "conversation-ai-config", async () => {
          ...
          const config = await page.evaluate(() => {
            ...
            document.querySelectorAll('[class*="toggle"], [class*="switch"], input[type="checkbox"]').forEach((el) => {
              const label =
                el.closest("[class*='field'], [class*='setting']")?.querySelector("label, [class*='label']")?.textContent?.trim() ||
        ...
        return await withPageError(page, "conversation-ai-update", async () => {
          ...
          if (args.tone) {
            const toneSelect = page
              .locator(
                '[class*="tone"] [class*="select"], select[name*="tone"], [class*="personality"] [class*="select"]',
              )
              .first();
            try {
          ...
          if (args.systemPrompt) {
            const promptInput = page
              .locator(
                'textarea[name="systemPrompt"], textarea[placeholder*="instruction"], textarea[placeholder*="prompt"], textarea',
              )
              .first();
            try {
      ...
      try {
        return await withPageError(page, "conversation-ai-training", async () => {
          ...
          const trainingTab = page
            .locator(
              'button:has-text("Training"), button:has-text("Knowledge"), [class*="tab"]:has-text("Training"), a:has-text("Training")',
            )
            .first();
          try {
          ...
          const entries = await page.evaluate((type: string) => {
            const items: Array<{
              title: string;
              type: string;
              question: string;
            ...
            for (const sel of rowSelectors) {
              document.querySelectorAll(sel).forEach((el) => {
                const titleEl = el.querySelector(
                  "h3, h4, [class*='title'], [class*='Title'], td:first-child",
                );
                const typeEl = el.querySelector(
                  "[class*='type'], [class*='Type'], [class*='badge'], td:nth-child(2)",
                );
                const questionEl = el.querySelector(
                  "[class*='question'], [class*='Question'], [class*='prompt']",
                );
                const answerEl = el.querySelector(
                  "[class*='answer'], [class*='Answer'], [class*='response']",
                );
                const urlEl = el.querySelector(
                  "a[href], [class*='url'], [class*='Url']",
                );
          ...
          const deduped = Array.from(
            new Map(entries.map((e) => [e.id || e.title || e.question, e])).values(),
          );
    ...
    ghl_browser_delete_ai_training_data: async (args) => {
      ...
      if (!confirm) throw new Error("confirm must be true to delete a training entry");
      if (!title && !id) throw new Error("entryTitle or entryId is required");
      ...
      try {
        return await withPageError(page, "conversation-ai-delete-training", async () => {
          ...
          const deleteBtn = row
            .locator(
              'button:has-text("Delete"), button:has-text("⋮"), button:has-text("⋯"), [class*="delete"]',
            )
            .first();
    ...
    ghl_browser_get_ai_conversation_logs: async (args) => {
      ...
      try {
        return await withPageError(page, "conversation-ai-logs", async () => {
          ...
          const logs = await page.evaluate((maxItems: number) => {
            const items: Array<{
              contact: string;
              channel: string;
              lastMessage: string;
            ...
            for (const sel of rowSelectors) {
              ...
              document.querySelectorAll(sel).forEach((el) => {
                ...
                const contactEl = el.querySelector(
                  "[class*='contact'], [class*='Contact'], [class*='name'], td:first-child",
                );
                const channelEl = el.querySelector(
                  "[class*='channel'], [class*='Channel'], [class*='type']",
                );
                const msgEl = el.querySelector(
                  "[class*='message'], [class*='Message'], [class*='last'], p",
                );
                const statusEl = el.querySelector(
                  "[class*='status'], [class*='Status'], [class*='badge']",
                );
                const dateEl = el.querySelector(
                  "[class*='date'], [class*='Date'], time, [class*='time']",
                );
                items.push({
