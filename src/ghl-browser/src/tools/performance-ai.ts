...
export const performanceAiModule: ToolModule = {
  tools: [
    {
      ...
      try {
        return await withPageError(page, "performance-suggestions", async () => {
          ...
          const suggestions = await page.evaluate(() => {
            const items: Array<{ label: string; description: string; category: string; status: string }> = [];
            document
      ...
      try {
        return await withPageError(page, "performance-scores", async () => {
          ...
          const scores = await page.evaluate(() => {
            const items: Array<{ name: string; score: string; type: string }> = [];
            document
