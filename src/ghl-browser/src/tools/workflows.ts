...
export const workflowsModule: ToolModule = {
  tools: [
    ...
    },
    {
      ...
      description:
        "Open a workflow and return a snapshot of its canvas: triggers, actions, and their connections.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description:
        "Add a trigger or action node to an open workflow canvas. Requires the workflow already open.",
      inputSchema: {
        type: "object",
        properties: {
  ...
  handlers: {
      ...
      try {
        return await withPageError(page, "workflows-list", async () => {
          ...
          const rows = await page.evaluate(() => {
            const items: Array<{ name: string; status: string; href: string }> = [];
            document
    ...
    ghl_browser_get_workflow_canvas: async (args) => {
      ...
      const id = args.workflowId as string | undefined;
      const { page, close } = await openPage();
      try {
