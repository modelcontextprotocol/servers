...
export const eventsModule: ToolModule = {
  ...
  handlers: {
      ...
      try {
        return await withPageError(page, "events-list", async () => {
          ...
          const rows = await page.evaluate(() => {
            const items: Array<{ name: string; date: string; type: string; attendees: string }> = [];
    ...
    ghl_browser_create_event: async (args) => {
      ...
      const type = (args.type as string) || "webinar";
      const { page, close } = await openPage();
      try {
        return await withPageError(page, "events-create", async () => {
          ...
          if (date) {
            try { await page.locator('input[type="date"], input[name*="date" i]').first().fill(date); } catch { /* optional */ }
          }
          if (description) {
    ...
    ghl_browser_list_event_registrations: async (args) => {
      ...
      try {
        return await withPageError(page, "events-registrations", async () => {
          ...
          await page.locator(`a:has-text("${eventName}"), [class*="name"]:has-text("${eventName}")`).first().click();
          await waitForAppReady(page);
          try { await page.locator('button:has-text("Registrations"), [role="tab"]:has-text("Registrations"), a:has-text("Attendees")').first().click({ timeout: 3000 }); } catch { /* tab may not exist */ }
          await waitForAppReady(page);
          const rows = await page.evaluate(() => {
            const items: Array<{ name: string; email: string; status: string; checkedIn: boolean }> = [];
    ...
    ghl_browser_delete_event: async (args) => {
      const eventName = String(args.eventName);
      if (!args.confirm) throw new Error("confirm: true is required to delete an event");
      const { page, close } = await openPage();
      try {
