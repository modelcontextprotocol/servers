...
export const copilotModule: ToolModule = {
  tools: [
    {
        ...
        return await withPageError(page, "copilot-status", async () => {
          ...
          const details = await page.evaluate(() => {
            ...
            document.querySelectorAll('input[type="checkbox"], [role="switch"], [class*="toggle"]').forEach((el) => {
              ...
              const checked = (el as HTMLInputElement).checked || el.getAttribute("aria-checked") === "true" || el.classList.toString().includes("active");
              if (label) toggles[label] = checked;
            });
            const fields: Record<string, string> = {};
            document.querySelectorAll('input:not([type="checkbox"]), select, textarea').forEach((el) => {
              const label = el.closest('label, [class*="field"]')?.querySelector('[class*="label"], span')?.textContent?.trim() || (el as HTMLInputElement).placeholder || "";
              if (label) fields[label] = (el as HTMLInputElement).value || "";
            });
      ...
      try {
        return await withPageError(page, "copilot-configure", async () => {
          ...
          if (typeof args.enabled === "boolean") {
            ...
            if (current !== args.enabled) await toggle.click();
          }
          if (typeof args.autoReply === "boolean") {
            try {
              ...
              if (current !== args.autoReply) await autoReplyToggle.click();
            } catch { /* setting may not exist */ }
          }
          await page.locator('button:has-text("Save")').first().click();
          await waitForAppReady(page);
          return { enabled: args.enabled ?? null, autoReply: args.autoReply ?? null, tone: (args.tone as string) || null, saved: true, url: page.url() };
        });
      } finally {
        await close();
      }
    ...
    ghl_browser_list_copilot_automations: async () => {
      ...
      try {
        return await withPageError(page, "copilot-automations", async () => {
          await gotoGhl(page, "/settings/copilot");
          await waitForAppReady(page);
          try { await page.locator('button:has-text("Automations"), [role="tab"]:has-text("Automations"), a:has-text("Rules")').first().click({ timeout: 3000 }); } catch { /* tab may not exist */ }
          await waitForAppReady(page);
          const rows = await page.evaluate(() => {
            const items: Array<{ name: string; trigger: string; status: string }> = [];
    ...
    ghl_browser_create_copilot_automation: async (args) => {
      ...
      try {
        return await withPageError(page, "copilot-create-automation", async () => {
          await gotoGhl(page, "/settings/copilot");
          await waitForAppReady(page);
          try { await page.locator('button:has-text("Automations"), [role="tab"]:has-text("Automations")').first().click({ timeout: 3000 }); } catch { /* optional */ }
          await waitForAppReady(page);
          ...
          if (prompt) {
            try { await page.locator('textarea, [contenteditable="true"]').first().fill(prompt); } catch { /* optional */ }
          }
    ...
    ghl_browser_get_copilot_logs: async (args) => {
      ...
      try {
        return await withPageError(page, "copilot-logs", async () => {
          await gotoGhl(page, "/settings/copilot");
          await waitForAppReady(page);
          try { await page.locator('button:has-text("Logs"), [role="tab"]:has-text("Logs"), a:has-text("Activity")').first().click({ timeout: 3000 }); } catch { /* tab may not exist */ }
          await waitForAppReady(page);
          const logs = await page.evaluate((max: number) => {
            const items: Array<{ time: string; action: string; contact: string; result: string }> = [];
