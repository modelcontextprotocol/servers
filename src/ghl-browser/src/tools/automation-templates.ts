...
export const automationTemplatesModule: ToolModule = {
  tools: [
    {
      name: "ghl_browser_list_automation_templates",
      description:
        "List available automation templates/recipes from the GHL marketplace or library. " +
      ...
      inputSchema: {
        ...
        properties: {
          ...
          category: {
            type: "string",
            description:
              "Filter by category: 'all' (default), 'lead_nurture', 'appointment', 'onboarding', 'follow_up', 'sales', 'marketing'",
          },
        },
      },
    },
    {
      ...
      description:
        "Get full details of an automation template: steps, triggers, actions, and required configuration.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description:
        "Install an automation template/recipe, creating a new workflow from the template's blueprint.",
      inputSchema: {
        type: "object",
        properties: {
          templateName: { type: "string" },
    ...
    },
    {
      ...
      description:
        "List automation recipes (multi-step sequences combining workflows, campaigns, and triggers). " +
    ...
    },
    {
      ...
      description:
        "Install an automation recipe that creates multiple linked assets " +
  ...
  handlers: {
        ...
        return await withPageError(page, "automation-templates-list", async () => {
          ...
          if (category !== "all") {
            const catFilter = page
              .locator(
                `button:has-text("${category}"), [class*="filter"]:has-text("${category}"), [class*="tab"]:has-text("${category}")`,
              )
              .first();
            try {
            ...
            for (const sel of cardSelectors) {
              document.querySelectorAll(sel).forEach((el) => {
                ...
                const catEl = el.querySelector(
                  "[class*='category'], [class*='Category'], [class*='badge'], [class*='tag']",
                );
                const stepsEl = el.querySelector(
                  "[class*='step'], [class*='Step'], [class*='node']",
                );
                const sourceEl = el.querySelector(
                  "[class*='source'], [class*='Source'], [class*='author']",
                );
          ...
          const deduped = Array.from(
            new Map(templates.map((t) => [t.id || t.name, t])).values(),
          );
      ...
      try {
        return await withPageError(page, "automation-templates-details", async () => {
          ...
          const details = await page.evaluate(() => {
            const steps: Array<{ name: string; type: string; description: string }> = [];
            document
              .querySelectorAll(
                '[class*="step"], [class*="Step"], [class*="node"], [class*="Node"], [class*="action-item"]',
              )
              .forEach((el) => {
                const nameEl = el.querySelector(
                  "[class*='name'], [class*='Name'], h3, h4, span:first-child",
                );
            ...
            document
              .querySelectorAll(
                '[class*="requirement"], [class*="Requirement"], [class*="prerequisite"], li',
              )
              .forEach((el) => {
            ...
            return {
              ...
              description:
                document.querySelector("[class*='description'], [class*='desc'], p")?.textContent?.trim() || "",
              steps,
      ...
      try {
        return await withPageError(page, "automation-templates-install", async () => {
          ...
          const installBtn = page
            .locator(
              'button:has-text("Install"), button:has-text("Use Template"), button:has-text("Import"), button:has-text("Apply")',
            )
            .first();
          ...
          const confirmBtn = page
            .locator(
              'button:has-text("Install"), button:has-text("Confirm"), button:has-text("Create"), button:has-text("Import")',
            )
            .first();
    ...
    ghl_browser_list_automation_recipes: async (args) => {
      ...
      try {
        return await withPageError(page, "automation-recipes-list", async () => {
          ...
          const recipes = await page.evaluate(() => {
            ...
            for (const sel of cardSelectors) {
              document.querySelectorAll(sel).forEach((el) => {
                ...
                const catEl = el.querySelector(
                  "[class*='category'], [class*='Category'], [class*='badge']",
                );
                const assetsEl = el.querySelector(
          ...
          const deduped = Array.from(
            new Map(recipes.map((r) => [r.id || r.name, r])).values(),
          );
    ...
    ghl_browser_install_automation_recipe: async (args) => {
      ...
      if (!name && !id) throw new Error("recipeName or recipeId is required");
      ...
      try {
        return await withPageError(page, "automation-recipes-install", async () => {
          ...
          const installBtn = page
            .locator(
              'button:has-text("Install"), button:has-text("Use Recipe"), button:has-text("Import")',
            )
            .first();
          await installBtn.click({ timeout: 5000 });
          await page.waitForTimeout(1500);
