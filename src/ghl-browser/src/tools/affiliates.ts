...
export const affiliateModule: ToolModule = {
  tools: [
    {
      name: "ghl_browser_list_affiliates",
      description:
        "List affiliates in the GHL affiliate manager. Returns affiliate name, email, " +
    ...
    },
    {
      ...
      description:
        "Create a new affiliate via the GHL UI. Returns the affiliate name and URL.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description:
        "Open an affiliate and return their full details: commission settings, referral links, " +
    ...
    },
    {
      ...
      description:
        "Update an affiliate's settings: commission rate, status, or other configuration.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description:
        "Get referral/tracking links for an affiliate. Returns the link URLs and click/conversion stats.",
      inputSchema: {
        type: "object",
        properties: {
          affiliateName: { type: "string" },
    ...
    },
    {
      ...
      description:
        "Delete an affiliate. This action is irreversible.",
      inputSchema: {
        type: "object",
        properties: {
          affiliateName: { type: "string" },
  ...
  handlers: {
      ...
      try {
        return await withPageError(page, "affiliates-list", async () => {
          ...
          const affiliates = await page.evaluate(() => {
            ...
            for (const sel of rowSelectors) {
              document.querySelectorAll(sel).forEach((el) => {
                ...
                const commissionEl = el.querySelector(
                  "[class*='commission'], [class*='Commission'], td:nth-child(3)",
                );
                const statusEl = el.querySelector(
                  "[class*='status'], [class*='Status'], [class*='badge']",
                );
                const referralsEl = el.querySelector(
                  "[class*='referral'], [class*='Referral'], td:nth-child(5)",
                );
                const revenueEl = el.querySelector(
                  "[class*='revenue'], [class*='Revenue'], td:nth-child(6)",
                );
                const id =
          ...
          const deduped = Array.from(
            new Map(affiliates.map((a) => [a.id || a.email || a.name, a])).values(),
          );
      ...
      try {
        return await withPageError(page, "affiliates-create", async () => {
          ...
          const commissionInput = page
            .locator(
              'input[name="commission"], input[placeholder*="Commission"], input[placeholder*="commission"]',
            )
            .first();
          try {
      ...
      try {
        return await withPageError(page, "affiliates-details", async () => {
          ...
          const details = await page.evaluate(() => {
            ...
            const links: Array<{ url: string; clicks: string; conversions: string }> = [];
            document
              .querySelectorAll(
                '[class*="link-row"], [class*="LinkRow"], [class*="referral-link"], tr:has(a[href*="ref"])',
              )
              .forEach((el) => {
            ...
            const payments: Array<{ date: string; amount: string; status: string }> = [];
            document
              .querySelectorAll(
                '[class*="payment-row"], [class*="PaymentRow"], tr:has([class*="payment"])',
              )
              .forEach((el) => {
      ...
      try {
        return await withPageError(page, "affiliates-update", async () => {
          ...
          if (commissionRate) {
            const commissionInput = page
              .locator(
                'input[name="commission"], input[placeholder*="Commission"], [class*="commission"] input',
              )
              .first();
            try {
          ...
          if (status) {
            ...
            try {
              ...
              await page
                .locator(
                  `[class*="option"]:has-text("${status}"), button:has-text("${status}")`,
                )
                .first()
                .click({ timeout: 3000 });
            } catch {
        ...
        return await withPageError(page, "affiliates-links", async () => {
          ...
          const linkData = await page.evaluate(() => {
            ...
            const allInputs = document.querySelectorAll(
              'input[value*="http"], input[value*="ref"], code, [class*="link"]',
            );
            allInputs.forEach((el) => {
              const url =
                (el as HTMLInputElement).value || el.textContent?.trim() || el.getAttribute("href") || "";
    ...
    ghl_browser_delete_affiliate: async (args) => {
      ...
      if (!confirm) throw new Error("confirm must be true to delete an affiliate");
      if (!name && !id) throw new Error("affiliateName or affiliateId is required");
      ...
      try {
        return await withPageError(page, "affiliates-delete", async () => {
          ...
          const menuBtn = row
            .locator(
              'button:has-text("⋮"), button:has-text("⋯"), [class*="menu"], [class*="actions"] button',
            )
            .first();
