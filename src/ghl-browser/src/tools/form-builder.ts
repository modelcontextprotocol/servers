...
export const formBuilderModule: ToolModule = {
  tools: [
    {
      name: "ghl_browser_list_forms",
      description:
        "List all forms (and optionally surveys) on the GHL Forms index page. " +
    ...
    },
    {
      ...
      description:
        "Create a new form or survey via the GHL UI. Returns the form name and URL after creation.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description:
        "Open a form in the builder and return its current field layout: field names, types, " +
    ...
    },
    {
      ...
      description:
        "Add a field to a form from the builder's field palette. " +
    ...
    },
    {
      ...
      description:
        "Save the current form from the builder. Optionally publish it to make it live.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description:
        "Delete a form from the forms index page. This action is irreversible.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description:
        "Get the embed code (HTML snippet, iframe URL, or direct link) for a form. " +
  ...
  handlers: {
    ghl_browser_list_forms: async (args) => {
      ...
      try {
        return await withPageError(page, "forms-list", async () => {
          ...
          const rows = await page.evaluate((filter: string) => {
            const items: Array<{ name: string; type: string; status: string; id: string; href: string }> = [];
            const rowSelectors = [
              "tr[data-row-key]",
              '[class*="ListRow"]',
              '[class*="form-row"]',
      ...
      try {
        return await withPageError(page, "forms-builder", async () => {
          ...
          const builder = await page.evaluate(() => {
            const fields: Array<{ label: string; type: string; required: boolean; order: number }> = [];
            ...
            document
              .querySelectorAll(
                '[class*="palette"] [class*="item"], [class*="sidebar"] [class*="field"], [class*="FieldList"] button',
              )
              .forEach((el) => {
      ...
      try {
        return await withPageError(page, "forms-add-field", async () => {
          ...
          const paletteItem = page
            .locator(
              `[class*="palette"] :has-text("${fieldType}"), ` +
    ...
    ghl_browser_delete_form: async (args) => {
      ...
      if (!confirm) throw new Error("confirm must be true to delete a form");
      if (!name && !id) throw new Error("formName or formId is required");
      ...
      try {
        return await withPageError(page, "forms-embed", async () => {
          ...
          const embedData = await page.evaluate(() => {
            ...
            return {
              html: codeEl?.textContent?.trim() || (codeEl as HTMLTextAreaElement)?.value || "",
              iframe: iframeEl?.textContent?.trim() || (iframeEl as HTMLInputElement)?.value || "",
              link: linkEl?.getAttribute("href") || (linkEl as HTMLInputElement)?.value || "",
            };
          });
