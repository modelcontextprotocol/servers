...
export const paymentsModule: ToolModule = {
  tools: [
    {
      ...
      try {
        return await withPageError(page, "payments-transactions", async () => {
          ...
          const rows = await page.evaluate(() => {
            const items: Array<{ id: string; contact: string; amount: string; status: string; method: string; date: string }> = [];
    ...
    ghl_browser_list_subscriptions: async () => {
      ...
      try {
        return await withPageError(page, "payments-subscriptions", async () => {
          ...
          await waitForAppReady(page);
          const rows = await page.evaluate(() => {
            const items: Array<{ contact: string; plan: string; amount: string; status: string; nextBilling: string }> = [];
    ...
    ghl_browser_list_payment_providers: async () => {
      ...
      try {
        return await withPageError(page, "payments-providers", async () => {
          ...
          await waitForAppReady(page);
          const rows = await page.evaluate(() => {
            const items: Array<{ name: string; status: string; type: string }> = [];
      ...
      try {
        return await withPageError(page, "payments-create-link", async () => {
          ...
          const link = await page.evaluate(() => {
            const linkEl = document.querySelector('a[href*="pay"], input[value*="http"]') as HTMLInputElement | HTMLAnchorElement | null;
            return linkEl?.getAttribute("href") || (linkEl as HTMLInputElement)?.value || "";
          });
