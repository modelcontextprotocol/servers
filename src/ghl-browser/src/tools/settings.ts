...
export const settingsModule: ToolModule = {
  tools: [
    {
      name: "ghl_browser_get_business_profile",
      description:
        "Get the sub-account business profile: name, address, phone, timezone, currency, and other settings.",
      inputSchema: {
        type: "object",
        properties: {},
      },
    },
    {
      ...
      description:
        "Update sub-account business profile fields: name, phone, address, timezone, etc.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description:
        "List users/team members for the current sub-account. Returns name, email, role, and status.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description:
        "Invite a new user/team member to the sub-account.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description:
        "Update a user's role or permissions in the sub-account.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description:
        "List configured integrations (Twilio, Mailgun, Stripe, etc.) and their connection status.",
      inputSchema: {
        type: "object",
        properties: {
          category: {
            type: "string",
            description:
              "Filter by category: 'all' (default), 'phone', 'email', 'payment', 'calendar', 'other'",
          },
        },
      },
    },
    {
      ...
      description:
        "Open an integration settings panel and update its configuration.",
      inputSchema: {
        type: "object",
        properties: {
      ...
      try {
        return await withPageError(page, "settings-profile", async () => {
          ...
          const profile = await page.evaluate(() => {
            ...
            const businessName =
              (document.querySelector('input[name="name"], input[name="businessName"]') as HTMLInputElement)?.value ||
    ...
    ghl_browser_update_business_profile: async (args) => {
      ...
      try {
        return await withPageError(page, "settings-profile-update", async () => {
          ...
          if (args.businessName) fieldMap["name"] = asString(args.businessName);
          if (args.phone) fieldMap["phone"] = asString(args.phone);
          if (args.address) fieldMap["address"] = asString(args.address);
          if (args.city) fieldMap["city"] = asString(args.city);
          if (args.state) fieldMap["state"] = asString(args.state);
          if (args.postalCode) fieldMap["postalCode"] = asString(args.postalCode);
          if (args.country) fieldMap["country"] = asString(args.country);
          if (args.timezone) fieldMap["timezone"] = asString(args.timezone);
          ...
          if (args.timezone) {
            const tzSelect = page
              .locator(
                'select[name*="timezone"], [class*="timezone"] select, [class*="Timezone"] [class*="select"]',
              )
              .first();
            try {
      ...
      try {
        return await withPageError(page, "settings-users", async () => {
            ...
            for (const sel of rowSelectors) {
              document.querySelectorAll(sel).forEach((el) => {
                const nameEl = el.querySelector(
                  "[class*='name'], [class*='Name'], td:first-child, h3, h4",
                );
                const emailEl = el.querySelector(
                  "[class*='email'], [class*='Email'], td:nth-child(2)",
                );
                const roleEl = el.querySelector(
                  "[class*='role'], [class*='Role'], td:nth-child(3), [class*='badge']",
                );
          ...
          const deduped = Array.from(
            new Map(users.map((u) => [u.id || u.email || u.name, u])).values(),
          );
      ...
      try {
        return await withPageError(page, "settings-create-user", async () => {
          ...
          const roleSelect = page
            .locator(
              '[class*="role"] [class*="select"], [class*="role"] select, select[name*="role"], [class*="Role"]',
            )
            .first();
          try {
      ...
      try {
        return await withPageError(page, "settings-integrations", async () => {
          ...
          if (category !== "all") {
            const catTab = page
              .locator(
                `[class*="tab"]:has-text("${category}"), button:has-text("${category}"), a:has-text("${category}")`,
              )
              .first();
            try {
          ...
          const integrations = await page.evaluate(() => {
            ...
            for (const sel of cardSelectors) {
              document.querySelectorAll(sel).forEach((el) => {
                ...
                const statusEl = el.querySelector(
                  "[class*='status'], [class*='Status'], [class*='badge'], [class*='connected']",
                );
                const descEl = el.querySelector(
                  "[class*='desc'], [class*='Desc'], p, small",
                );
                const categoryEl = el.querySelector("[class*='category'], [class*='Category'], [class*='tag']");
                const isConfigured =
                  !!el.querySelector("[class*='connected'], [class*='configured'], [class*='active']") ||
          ...
          const deduped = Array.from(
            new Map(integrations.map((i) => [i.name, i])).values(),
          );
    ...
    ghl_browser_configure_integration: async (args) => {
      ...
      try {
        return await withPageError(page, "settings-configure-integration", async () => {
          ...
          const card = page
            .locator(
              `[class*="card"]:has-text("${name}"), [class*="IntegrationItem"]:has-text("${name}"), ` +
          ...
          if (apiKey) {
            const keyInput = page
              .locator(
                'input[name="apiKey"], input[name="api_key"], input[placeholder*="API"], input[type="password"], input[type="text"]',
              )
              .first();
            try {
