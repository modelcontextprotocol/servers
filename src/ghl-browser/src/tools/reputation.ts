...
export const reputationModule: ToolModule = {
  tools: [
    {
      name: "ghl_browser_list_reviews",
      description:
        "List reviews from connected platforms (Google, Facebook, etc.) with reviewer name, " +
    ...
    },
    {
      ...
      description:
        "Send a review request to a contact via SMS or email. Uses GHL's built-in review funnel.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description:
        "Reply to a review. The response is posted to the original platform (Google, Facebook, etc.).",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description:
        "Get the overall reputation score: average rating, total review count, " +
    ...
    },
    {
      ...
      description:
        "List connected review platforms/sites with name, URL, review count, " +
        ...
        return await withPageError(page, "reputation-score", async () => {
          ...
          const score = await page.evaluate(() => {
            ...
            document.querySelectorAll('[class*="rating-bar"], [class*="RatingBar"], [class*="distribution"] [class*="row"]').forEach((el) => {
              ...
              if (starLabel) ratingDistribution[starLabel] = count;
            });

            const platformScores: Array<{ name: string; rating: string; count: string }> = [];
      ...
      try {
        return await withPageError(page, "review-sites", async () => {
          ...
          const sites = await page.evaluate(() => {
            const items: Array<{ name: string; url: string; reviewCount: string; avgRating: string; connected: boolean }> = [];
            const siteSelectors = [
              '[class*="site-item"]',
              '[class*="SiteItem"]',
              '[class*="platform-item"]',
            ...
            for (const sel of siteSelectors) {
              document.querySelectorAll(sel).forEach((el) => {
                ...
                items.push({
                  ...
                  url: (urlEl as HTMLAnchorElement)?.href || urlEl?.textContent?.trim() || "",
