...
export const conversationsModule: ToolModule = {
  tools: [
    {
  ...
  handlers: {
      ...
      try {
        return await withPageError(page, "conversations-list", async () => {
          ...
          const rows = await page.evaluate(() => {
            const items: Array<{ contact: string; lastMessage: string; channel: string; unread: boolean; time: string }> = [];
    ...
    ghl_browser_get_conversation_thread: async (args) => {
      ...
      const maxMessages = (args.maxMessages as number) || 50;
      const { page, close } = await openPage();
      try {
        return await withPageError(page, "conversations-thread", async () => {
          ...
          const messages = await page.evaluate((max: number) => {
            const msgs: Array<{ sender: string; text: string; time: string; direction: string }> = [];
