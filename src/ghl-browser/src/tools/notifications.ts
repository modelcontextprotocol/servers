      ...
      try {
        return await withPageError(page, "notifications-list", async () => {
          ...
          const items = await page.evaluate(() => {
            const rows: Array<{ text: string; time: string; unread: boolean }> = [];
            document
      ...
      try {
        return await withPageError(page, "notifications-settings", async () => {
          ...
          const settings = await page.evaluate(() => {
            const items: Array<{ label: string; enabled: boolean; channel: string }> = [];
            document.querySelectorAll('input[type="checkbox"], [role="switch"], [role="checkbox"]').forEach((el) => {
              const label = (el as HTMLElement).closest("label, [class*='row'], [class*='item']")?.textContent?.trim().slice(0, 80) || "";
              const checked =
