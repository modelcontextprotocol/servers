      ...
      try {
        return await withPageError(page, "contacts-list", async () => {
          ...
          const rows = await page.evaluate(() => {
            const items: Array<{ name: string; email: string; phone: string; tags: string[] }> = [];
    ...
    ghl_browser_create_contact_browser: async (args) => {
      ...
      try {
        return await withPageError(page, "contacts-create", async () => {
          ...
          await waitForAppReady(page);
          if (firstName) await page.locator('input[name*="first" i], input[placeholder*="First"]').first().fill(firstName);
          if (lastName) await page.locator('input[name*="last" i], input[placeholder*="Last"]').first().fill(lastName);
          if (email) await page.locator('input[type="email"], input[name="email"]').first().fill(email);
          if (phone) await page.locator('input[type="tel"], input[name="phone"]').first().fill(phone);
          if (tags) {
    ...
    ghl_browser_edit_contact: async (args) => {
      ...
      try {
        return await withPageError(page, "contacts-edit", async () => {
          ...
          await page.locator(`a:has-text("${contactName}"), [class*="name"]:has-text("${contactName}")`).first().click();
          await waitForAppReady(page);
          if (args.firstName) await page.locator('input[name*="first" i]').first().fill(String(args.firstName));
          if (args.lastName) await page.locator('input[name*="last" i]').first().fill(String(args.lastName));
          if (args.email) await page.locator('input[type="email"]').first().fill(String(args.email));
          if (args.phone) await page.locator('input[type="tel"]').first().fill(String(args.phone));
    ...
    ghl_browser_list_smart_lists: async () => {
      ...
      try {
        return await withPageError(page, "contacts-smartlists", async () => {
          ...
          await waitForAppReady(page);
          const rows = await page.evaluate(() => {
            const items: Array<{ name: string; count: string; filters: string }> = [];
