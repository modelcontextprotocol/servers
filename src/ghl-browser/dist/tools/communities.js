...
export const communitiesModule = {
    ...
    handlers: {
        ...
        ghl_browser_list_community_members: async (args) => {
            ...
            try {
                return await withPageError(page, "communities-members", async () => {
                    ...
                    await waitForAppReady(page);
                    try {
                        await page.locator('button:has-text("Members"), [role="tab"]:has-text("Members")').first().click({ timeout: 3000 });
                    }
        ...
        ghl_browser_delete_community: async (args) => {
            const communityName = String(args.communityName);
            if (!args.confirm)
                throw new Error("confirm: true is required to delete a community");
            const { page, close } = await openPage();
            try {
