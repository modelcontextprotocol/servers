    ...
    tools: [
        {
            ...
            description: "List available account snapshots (templates). Returns name, description, creation date, and included assets.",
            inputSchema: {
                type: "object",
                properties: {
        ...
        },
        {
            ...
            description: "Create a snapshot from the current sub-account, capturing workflows, funnels, pipelines, " +
            ...
            inputSchema: {
                ...
                properties: {
                    ...
                    includeAssets: {
                        type: "string",
                        description: "Comma-separated list of asset types to include: workflows, funnels, pipelines, campaigns, calendars, forms, emails, tags, custom_fields, triggers. Default: all",
                    },
                },
                required: ["name"],
            },
        },
        {
            ...
            description: "Get detailed information about a snapshot: included assets, creation date, and size.",
            inputSchema: {
                type: "object",
                properties: {
        ...
        },
        {
            ...
            description: "Load (apply) a snapshot to a target sub-account. This will create or overwrite assets in the target.",
            inputSchema: {
                type: "object",
                properties: {
                    snapshotName: { type: "string" },
        ...
        },
        {
            ...
            description: "Delete a snapshot. This action is irreversible.",
            inputSchema: {
                type: "object",
                properties: {
                    snapshotName: { type: "string" },
    ...
    handlers: {
        ghl_browser_list_snapshots: async (args) => {
            ...
            try {
                return await withPageError(page, "snapshots-list", async () => {
                    ...
                    const snapshots = await page.evaluate(() => {
                        ...
                        for (const sel of rowSelectors) {
                            document.querySelectorAll(sel).forEach((el) => {
                                ...
                                const descEl = el.querySelector("[class*='desc'], [class*='Desc'], p, small, td:nth-child(2)");
                                const typeEl = el.querySelector("[class*='type'], [class*='Type'], [class*='badge']");
                                const dateEl = el.querySelector("[class*='date'], [class*='Date'], [class*='created'], time");
                                const assetEl = el.querySelector("[class*='asset'], [class*='Asset'], [class*='includes']");
                    ...
                    const deduped = Array.from(new Map(snapshots.map((s) => [s.id || s.name, s])).values());
                ...
                return await withPageError(page, "snapshots-create", async () => {
                    ...
                    if (includeAssets) {
                        ...
                        for (const assetType of assetTypes) {
                            const checkbox = page
                                .locator(`label:has-text("${assetType}") input[type="checkbox"], ` +
            ...
            try {
                return await withPageError(page, "snapshots-details", async () => {
                    ...
                    const details = await page.evaluate(() => {
                        ...
                        const assets = [];
                        document
                            .querySelectorAll('[class*="asset-item"], [class*="AssetItem"], [class*="asset-row"], ' +
        ...
        ghl_browser_load_snapshot: async (args) => {
            ...
            if (!targetId && !targetName)
                throw new Error("targetAccountId or targetAccountName is required");
            const { page, close } = await openPage();
            try {
                return await withPageError(page, "snapshots-load", async () => {
                    ...
                    const targetInput = page
                        .locator('input[placeholder*="Search"], input[placeholder*="account"], input[placeholder*="Account"], [class*="search"] input')
                        .first();
                    try {
                        await targetInput.fill(targetName || targetId, { timeout: 5000 });
                        await page.waitForTimeout(1000);
                        await page
                            .locator(`[class*="option"]:has-text("${targetName || targetId}"), [class*="result"]:has-text("${targetName || targetId}")`)
                            .first()
                            .click({ timeout: 5000 });
                    }
                    catch {
                    ...
                    if (overwrite) {
                        const overwriteToggle = page
                            .locator('label:has-text("Overwrite"), [class*="overwrite"], input[name="overwrite"]')
                            .first();
                        try {
        ...
        ghl_browser_delete_snapshot: async (args) => {
            ...
            if (!confirm)
                throw new Error("confirm must be true to delete a snapshot");
            if (!name && !id)
                throw new Error("snapshotName or snapshotId is required");
            const { page, close } = await openPage();
            try {
