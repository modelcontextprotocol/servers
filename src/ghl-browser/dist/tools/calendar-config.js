...
export const calendarConfigModule = {
    tools: [
        {
            name: "ghl_browser_list_calendars",
            description: "List appointment calendars with name, type (simple/round_robin/group), " +
        ...
        },
        {
            ...
            description: "Open a calendar and return its full configuration: slot duration, " +
        ...
        },
        {
            ...
            description: "Create a new appointment calendar. Supports simple (single user), " +
        ...
        },
        {
            ...
            description: "Update the availability windows for a calendar: set open/close times per day of week. " +
            ...
            inputSchema: {
                ...
                properties: {
                    ...
                    availability: {
                        type: "object",
                        description: "Map of day names to time ranges. Example: " +
        ...
        },
        {
            ...
            description: "Assign or remove users from a calendar. For round-robin calendars, this controls " +
        ...
        },
        {
            ...
            description: "Get the public booking URL for a calendar, suitable for sharing with leads " +
        ...
        },
        {
            ...
            description: "Delete an appointment calendar. This action is irreversible.",
            inputSchema: {
                type: "object",
                properties: {
                    calendarName: { type: "string" },
    ...
    handlers: {
            ...
            try {
                return await withPageError(page, "calendar-config", async () => {
                    ...
                    const config = await page.evaluate(() => {
                        ...
                        const availability = {};
                        const dayRows = document.querySelectorAll('[class*="availability"] [class*="row"], ' +
                        ...
                        dayRows.forEach((el) => {
                            ...
                            if (dayText)
                                availability[dayText.toLowerCase()] = timeSlots;
                        });
                        const assignedUsers = [];
                        document
                ...
                return await withPageError(page, "calendar-availability", async () => {
                    ...
                    for (const [day, slots] of Object.entries(availability)) {
                        ...
                        for (let i = 0; i < slots.length; i++) {
                            ...
                            if (!start || !end)
                                continue;
                            const timeInputs = page.locator(`[data-day="${day}"] input[type="time"], [class*="${day}"] input[type="time"]`);
            ...
            try {
                return await withPageError(page, "calendar-booking-link", async () => {
                    ...
                    const linkData = await page.evaluate(() => {
                        const linkInput = document.querySelector('input[value*="book"], input[value*="calendar"], input[placeholder*="link"], input[readonly]');
                        const linkAnchor = document.querySelector('a[href*="book"], a[href*="calendar"], a[href*="schedule"]');
                        return {
        ...
        ghl_browser_delete_calendar: async (args) => {
            ...
            if (!confirm)
                throw new Error("confirm must be true to delete a calendar");
            if (!name && !id)
                throw new Error("calendarName or calendarId is required");
            const { page, close } = await openPage();
            try {
