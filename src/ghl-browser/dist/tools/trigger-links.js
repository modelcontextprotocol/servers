...
export const triggerLinksModule = {
    tools: [
        {
            name: "ghl_browser_list_trigger_links",
            description: "List trigger (tracking) links. Returns link name, URL, click count, and associated action.",
            inputSchema: {
                type: "object",
                properties: {
        ...
        },
        {
            ...
            description: "Create a new trigger link that fires an action (workflow, tag, pipeline update) when clicked.",
            inputSchema: {
                type: "object",
                properties: {
                    ...
                    actionType: {
                        type: "string",
                        description: "Action to trigger: 'workflow', 'add_tag', 'remove_tag', 'update_pipeline', 'add_to_campaign', 'custom'",
                    },
                    actionValue: {
                        type: "string",
        ...
        },
        {
            ...
            description: "Get click analytics for a trigger link: total clicks, unique clicks, conversions, and recent activity.",
            inputSchema: {
                type: "object",
                properties: {
        ...
        },
        {
            ...
            description: "Update a trigger link's name, action, or redirect URL.",
            inputSchema: {
                type: "object",
                properties: {
        ...
        },
        {
            ...
            description: "Delete a trigger link. This action is irreversible.",
            inputSchema: {
                type: "object",
                properties: {
                    linkName: { type: "string" },
    ...
    handlers: {
        ghl_browser_list_trigger_links: async (args) => {
            ...
            try {
                return await withPageError(page, "triggers-list", async () => {
                    ...
                    const links = await page.evaluate(() => {
                        ...
                        for (const sel of rowSelectors) {
                            document.querySelectorAll(sel).forEach((el) => {
                                ...
                                const urlEl = el.querySelector("[class*='url'], [class*='Url'], a[href], code, input[value]");
                                const clickEl = el.querySelector("[class*='click'], [class*='Click'], td:nth-child(3)");
                                const actionEl = el.querySelector("[class*='action'], [class*='Action'], td:nth-child(4), [class*='badge']");
                                const statusEl = el.querySelector("[class*='status'], [class*='Status'], [class*='badge']");
                                const id = el.getAttribute("data-row-key") ||
                                    el.getAttribute("data-id") ||
                                    "";
                                const urlValue = urlEl?.value ||
                    ...
                    const deduped = Array.from(new Map(links.map((l) => [l.id || l.name || l.url, l])).values());
                ...
                return await withPageError(page, "triggers-create", async () => {
                    ...
                    if (actionType) {
                        const actionSelect = page
                            .locator('[class*="action"] [class*="select"], select[name*="action"], [class*="Action"] select')
                            .first();
                        try {
                            await actionSelect.click({ timeout: 5000 });
                            await page
                                .locator(`[class*="option"]:has-text("${actionType}"), button:has-text("${actionType}")`)
                                .first()
                                .click({ timeout: 3000 });
                        }
                        catch {
                    ...
                    if (actionValue) {
                        const valueInput = page
                            .locator('[class*="value"] input, input[name="actionValue"], input[placeholder*="Select"], [class*="search"] input')
                            .first();
                        try {
                    ...
                    if (redirectUrl) {
                        const redirectInput = page
                            .locator('input[name="redirectUrl"], input[placeholder*="Redirect"], input[placeholder*="URL"]')
                            .first();
                        try {
            ...
            try {
                return await withPageError(page, "triggers-stats", async () => {
                    ...
                    const stats = await page.evaluate(() => {
                        ...
                        const activity = [];
                        document
                            .querySelectorAll('[class*="activity-row"], [class*="ActivityRow"], tr:has([class*="click"])')
                            .forEach((el) => {
        ...
        ghl_browser_delete_trigger_link: async (args) => {
            ...
            if (!confirm)
                throw new Error("confirm must be true to delete a trigger link");
            if (!name && !id)
                throw new Error("linkName or linkId is required");
            const { page, close } = await openPage();
            try {
