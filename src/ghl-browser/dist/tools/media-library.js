...
export const mediaLibraryModule = {
    tools: [
        {
            ...
            description: "List files in the GHL media library. Returns file name, type, size, and URL.",
            inputSchema: {
                type: "object",
                properties: {
        ...
        },
        {
            ...
            description: "Upload a file to the GHL media library from a local path or URL.",
            inputSchema: {
                type: "object",
                properties: {
        ...
        },
        {
            ...
            description: "Get details of a specific media file: dimensions, URL, embed code, usage count.",
            inputSchema: {
                type: "object",
                properties: {
        ...
        },
        {
            ...
            description: "Create a new folder in the media library for organizing files.",
            inputSchema: {
                type: "object",
                properties: {
        ...
        },
        {
            ...
            description: "Delete a media file. This action is irreversible.",
            inputSchema: {
                type: "object",
                properties: {
    ...
    handlers: {
        ghl_browser_list_media: async (args) => {
            ...
            try {
                return await withPageError(page, "media-list", async () => {
                    ...
                    const files = await page.evaluate(() => {
                        ...
                        for (const sel of cardSelectors) {
                            document.querySelectorAll(sel).forEach((el) => {
                                const nameEl = el.querySelector("[class*='name'], [class*='Name'], [class*='title'], span, p");
                                const typeEl = el.querySelector("[class*='type'], [class*='Type'], [class*='ext'], [class*='badge']");
                                const sizeEl = el.querySelector("[class*='size'], [class*='Size']");
                    ...
                    const deduped = Array.from(new Map(files.map((f) => [f.id || f.name, f])).values());
        ...
        ghl_browser_upload_media: async (args) => {
            ...
            if (!filePath && !fileUrl)
                throw new Error("filePath or fileUrl is required");
            const { page, close } = await openPage();
            try {
        ...
        ghl_browser_delete_media: async (args) => {
            ...
            if (!confirm)
                throw new Error("confirm must be true to delete a media file");
            if (!name && !id)
                throw new Error("fileName or fileId is required");
            const { page, close } = await openPage();
            try {
