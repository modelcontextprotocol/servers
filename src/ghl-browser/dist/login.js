...
async function main() {
    ...
    console.log("[ghl-browser-login] IMPORTANT: complete the FULL login in the browser window (password + any 2FA/verification step).");
    console.log("[ghl-browser-login] When you can see your GHL dashboard, come back to THIS terminal and press ENTER.");
    ...
    const stillLogin = await page
        .evaluate(() => /\/(login|sign-in|signin)|oauth2|two-factor|2fa|verify/i.test(location.href) ||
        !!document.querySelector("input[type=password]"))
        .catch(() => false);
    if (stillLogin) {
        console.error(`[ghl-browser-login] Browser still shows a login/2FA page (${finalUrl}). Finish logging in, then run this script again.`);
        await browser.close();
        process.exit(1);
    }
