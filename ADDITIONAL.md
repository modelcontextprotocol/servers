...
* **[Knowledge Fabric](https://github.com/sagarv48/knowledge-fabric)** (Python) - Vendor-neutral, multi-tenant hybrid RAG server powered by PostgreSQL + pgvector and full-text search (BM25 RRF). Exposes `retrieve_evidence`, `get_document`, `explain_retrieval`, and `health_check` tools to any MCP client.
