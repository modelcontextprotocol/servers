...
- **[Magertron's MCP Orchestrator](https://magertron.com)** - Self-hosted MCP orchestration platform with SSO, RBAC, OCSF audit trails, and inventory plane for SIEM/MDM integration. Includes dynamic MCP server discovery, Envoy data-plane routing, and token-gated external access. Deploy via Helm on Kubernetes.
