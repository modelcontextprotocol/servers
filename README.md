...
- **[Not Human Search](https://nothumansearch.ai)** ([MCP endpoint](https://nothumansearch.ai/mcp), [API](https://nothumansearch.ai/api/v1/search?q=)) - Search engine that indexes MCP servers and other agent-first sites, ranking them by agentic readiness across 7 signals (llms.txt, OpenAPI, ai-plugin, MCP, structured API, robots.txt, schema.org). 8,000+ sites indexed; includes a live `verify_mcp` tool that probes any URL to confirm it's a responding MCP endpoint. By **[unitedideas](https://github.com/unitedideas)**
